...
/**
 * Maps feature variant by external_id in case it is defined for the feature variant.
 *
 ...
 * @return void
 */
function fn_rus_exim_1c_update_product_feature_variant_before_select(
    $feature_id,
    $feature_type,
    $variant,
    $variant_id,
    ...
    if (!isset($variant['external_id']) || !isset($variant['commerceml'])) {
        return;
    }
    $conditions['external_id'] = db_quote('fv.external_id = ?i', $variant['external_id']);
    unset($conditions['variant']);
}
