...
class Pickpoint implements IService
{
    ...
    public function getRequestData()
    {
        $data = array();
        $key_data = array();
        $pickpoint_info = isset($this->_shipping_info['service_params']['pickpoint_info']) ? $this->_shipping_info['service_params']['pickpoint_info'] : [];
        ...
        if ($login) {
            ...
            if (isset($cities[$origination['state']])) {
                $state = $cities[$origination['state']];
            }
            $from_state = !empty($state) && is_array($state) ? reset($state)['state'] : null;
