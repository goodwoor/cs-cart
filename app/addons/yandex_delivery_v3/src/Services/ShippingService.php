...
class ShippingService
{
    ...
    public function formatPackagesDimensions(array $package_info)
    {
        ...
        foreach ($package_info['packages'] as $package) {
            ...
            if (
                !$this->isProductDimensionsExist(
                    [
                        'length' => ceil((float) $package['shipping_params']['box_length']),
                        'width'  => ceil((float) $package['shipping_params']['box_width']),
                        'height' => ceil((float) $package['shipping_params']['box_height']),
    ...
    public function getPickupPoints(array $data)
    {
        ...
        foreach ($data as $delivery) {
            ...
            $pickup_points = array_merge($pickup_points, $delivery['pickupPointIds']);
        }
